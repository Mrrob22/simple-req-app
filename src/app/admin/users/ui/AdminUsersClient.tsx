...
type CreateUserResp =
    | { ok: true; id: string; role: UserRole }
    | { ok: false; error: string };
...
export default function AdminUsersClient() {
    ...
    async function load() {
        ...
        const data = (await res.json()) as ListUsersResp;
        setLoading(false);
    ...
    async function createUser() {
        ...
        const data = (await res.json()) as CreateUserResp;
        if (!data.ok) {
            setErr(data.error || 'Помилка створення');
            return;
        }
    ...
    async function changeRole(id: string, role: UserRole) {
        ...
        const data = (await res.json()) as ChangeRoleResp;
        if (!data.ok) {
            setErr(data.error || 'Помилка зміни ролі');
            return;
        }
        void load();
    }
        ...
        <main style={{ maxWidth: 1000, margin: '40px auto', padding: 16 }}>
            ...
            <section style={{ border: '1px solid #eee', padding: 12, borderRadius: 8, marginTop: 12 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 160px auto', gap: 8, alignItems: 'end' }}>
                    ...
                    <button onClick={createUser} disabled={!form.name || !form.email || form.password.length < 8}>
                        Створити
                    </button>
                </div>
            </section>
